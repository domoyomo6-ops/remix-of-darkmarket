...
export default function ProductDetail() {
  ...
  const handlePurchase = async () => {
    ...
    if (error) {
      ...
      const result = data as { success: boolean; error?: string; product_title?: string };
      if (result.success) {
    ...
    <MainLayout>
      <div className="relative min-h-screen">
        ...
        <div className="container mx-auto px-4 py-8 relative z-10">
          ...
          <button
            onClick={() => navigate(-1)}
            className="mb-6 text-emerald-500/60 hover:text-emerald-400 transition-colors font-mono text-sm flex items-center gap-2 group"
          >
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="relative group">
              <div className="absolute -inset-[1px] bg-gradient-to-r from-emerald-500/20 via-transparent to-cyan-500/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="mb-4 inline-flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className={`uppercase font-mono text-[10px] tracking-wider ${categoryColors[product.category] || ''}`}
                  >
                    {product.category}
                  </Badge>
                </div>
                ...
                {product.short_description && (
                  <p className="text-zinc-400 font-mono text-sm leading-relaxed border-l-2 border-emerald-500/20 pl-4">
                    {product.short_description}
                  </p>
                )}
              </div>
              ...
              <div className="flex items-baseline gap-3 py-4 border-y border-emerald-500/10">
                ...
                <span className="text-4xl font-mono font-bold text-emerald-400" style={{ textShadow: '0 0 20px rgba(16,185,129,0.3)' }}>
                  ${product.price}
                </span>
              ...
              {hasPurchased ? (
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-4 rounded border border-emerald-500/40 bg-emerald-500/10 text-emerald-400 font-mono text-sm uppercase tracking-wider hover:bg-emerald-500/20 hover:border-emerald-500/60 transition-all flex items-center justify-center gap-2 group"
                  >
                ...
                <button
                  ...
                  disabled={purchasing}
                  className="w-full py-4 rounded border border-emerald-500/40 bg-emerald-500/10 text-emerald-400 font-mono text-sm uppercase tracking-wider hover:bg-emerald-500/20 hover:border-emerald-500/60 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
