...
export default function SupportChatBox() {
  ...
  const [newOrderCount, setNewOrderCount] = useState(0); // new orders badge

  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Chat: existing chat + support status
  ...
  // Food Orders: realtime subscription
  useEffect(() => {
    if (!user) return;
    const channel = supabase
      .channel(`food_orders_${user.id}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'food_orders', filter: `user_id=eq.${user.id}` },
        () => setNewOrderCount(prev => prev + 1) // increment badge when new order arrives
      ).subscribe();
    return () => supabase.removeChannel(channel);
  }, [user]);
  ...
  return (
    <>
      {/* Floating Button */}
      <button
        onClick={() => { setIsOpen(true); setIsMinimized(false); markAsRead(); setNewOrderCount(0); }}
        className={`fixed bottom-4 right-4 z-50 w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-lg 
          ...
          ${hasNewMessage || newOrderCount > 0 ? 'animate-bounce' : ''} 
          ...
          {!isMinimized && (
            <Tabs defaultValue="chat" className="h-[calc(100%-48px)] flex flex-col">
              ...
              <TabsList className="flex shrink-0 bg-black/50 border-b border-primary/20">
                ...
                <TabsTrigger value="orders" className="flex-1 font-mono text-xs">
                  🍔 Orders {newOrderCount > 0 && <span className="ml-1 w-4 h-4 rounded-full bg-red-500 text-white text-[10px] flex items-center justify-center">{newOrderCount}</span>}
                </TabsTrigger>
              </TabsList>
